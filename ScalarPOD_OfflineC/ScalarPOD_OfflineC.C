...
#include "setRootCase.H"
    ...
    bool isMultiDir = POD_parameters.isMultiDir;
...
//Write out all matrices

scalarRectangularMatrix H  (scalarPODOrthoNormalBasis.size(),1 , Foam::zero());

for (label ii=0; ii< scalarPODOrthoNormalBasis.size(); ii++)
{	        
		H[ii][0]=fvc::domainIntegrate(scalarPODOrthoNormalBasis[ii]).value();
}

std::ofstream f5 ("Hmatrix.txt");  //che poi questo sarebbe un vettore in realtà

if (f5.is_open())
{
	for (label ii=0; ii<scalarPODOrthoNormalBasis.size(); ii++)
	{
		f5 << std::setprecision(14) <<H(ii,0)<< " ";
		f5 << "\n";
	}
	f5.close();	
}

//Info << "Matrix H is " << H <<endl; 

scalarRectangularMatrix A  (scalarPODOrthoNormalBasis.size(),1 , Foam::zero());

for (label ii=0; ii< scalarPODOrthoNormalBasis.size(); ii++)
{	        
		A[ii][0]=fvc::domainIntegrate(scalarPODOrthoNormalBasis[ii]).value();
}

std::ofstream f6 ("Amatrix.txt");  //che poi questo sarebbe un vettore in realtà

if (f6.is_open())
{
	for (label ii=0; ii<scalarPODOrthoNormalBasis.size(); ii++)
	{
		f6 << std::setprecision(14) <<A(ii,0)<< " ";
		f6 << "\n";
	}
	f6.close();	
}

//Info << "Matrix A is " << A <<endl; 

scalarRectangularMatrix S (scalarPODOrthoNormalBasis.size(),scalarPODOrthoNormalBasis.size(), Foam::zero());
...
for (label ii=0; ii< scalarPODOrthoNormalBasis.size(); ii++)
{
	for (label jj=0; jj< scalarPODOrthoNormalBasis.size(); jj++)
		...
		{
			SurfaceIntegral+=((mesh.boundary()[mesh.boundary().findPatchID("atmosphere")].magSf()[SfI])*(scalarPODOrthoNormalBasis[ii].boundaryField()[ mesh.boundary().findPatchID("atmosphere")][SfI])*(scalarPODOrthoNormalBasis[jj].boundaryField()[ mesh.boundary().findPatchID("atmosphere")][SfI]));
		}
		S[ii][jj]=SurfaceIntegral;
...
if (f7.is_open())
{
	for (label ii=0; ii<scalarPODOrthoNormalBasis.size(); ii++)
	{
		for (label jj=0; jj<scalarPODOrthoNormalBasis.size(); jj++ )
		{
			f7 << std::setprecision(14) <<S(ii,jj)<< " ";
		}
		f7 << "\n";
	}
	f7.close();	
